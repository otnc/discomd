...
| `npm run test` | Run the tests once with mocha (`npm run test:watch` to keep them running) |
...
- **Tests live next to the code** as `*.test.ts` and run with mocha (via `ts-node`'s ESM loader).
